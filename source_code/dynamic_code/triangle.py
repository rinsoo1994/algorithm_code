    ...
    while queue:
        ...
        for i in range(m):
            for allow in allow_direction:
                ...
                if result_list[x][y] + init_list[n_x][n_y] >= result_list[x][y]:
                    queue.append((n_x, n_y))
                    result_list[n_x][n_y] = result_list[x][y] + init_list[n_x][n_y]

    print(init_list)
    print(result_list)
    result = 0
    for num in range(n):
        result = max(result, result_list[num][m-1])
    print(result)